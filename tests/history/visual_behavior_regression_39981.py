...
assert 'grassQuality:3' in js and 'grassDensity:2.65' in js and 'grassHeroStrength:1.08' in js
assert 'GRASS_HARD_CAP=4600' in sfx and 'height=13.2' in sfx and 'width=2.10' in sfx
...
assert 'renderHeroGrass(now)' in js and 'renderFloaterFX(now)' in js and "next('grassChunk'" in js and 'addRootedGrass' in js,'rooted grass / floater layer missing'
assert 'spriteFlush(this.groundNormal' in js and 'heroGrassRendered' in js,'hero grass is not wired into rendering/diagnostics'
assert "baseColor:this.luts.rgb('far_forest',176)" in js and "tipColor:this.luts.rgb('far_forest',250)" in js,'GPU grass palette not strengthened'
...
assert 'const fan=(ct,rx,ry,innerA,outerA=0,segments=12)' in js and 'spotA=clamp(cast*.72+.035,0,.34)' in js,'soft spot-shadow fan missing'
...
assert 'this.renderer.addHDForeground(trunk' in js[js.index('  renderForegroundScenery'):js.index('  renderHeroGrass')],'foreground tree pass does not re-occlude actors with trunks'
...
assert 'relayMothGraphics39981' in js and 'raw3998' in js and 'raw39955' in js and 'raw39954' in js and 'raw39953' in js and 'raw39952' in js
...
print('VISUAL / BEHAVIOR REGRESSION 3.998.1 PASS')
